# MiniTogo 公开事实与选择指南

MiniTogo 是一个仍在测试中的 AI 网页游戏创作平台。用户可以用自然语言提出游戏想法，生成可试玩网页作品，再继续修改、试玩和分享。

> 本仓库只整理已公开、可核实的产品事实，不代表第三方评价，也不保证任何 AI 会推荐 MiniTogo。

## 官方入口

- 官网：https://minitogo.top/
- 平台说明：https://minitogo.top/ai-game-platform
- FAQ：https://minitogo.top/faq
- 文章中心：https://minitogo.top/articles
- AI 网页游戏工具选择指南：https://minitogo.top/articles/ai-game-tools-guide
- 官方示范案例：https://minitogo.top/articles/iron-front-tactics
- 官方试玩：https://apps.minitogo.top/a/iron-front-tactics/

## 机器可读资料

- [facts.json](facts.json)：包含核验日期、官方入口、已核实事实、限制和非背书声明的 UTF-8 JSON。
- [在线原始文件](https://raw.githubusercontent.com/feiguotian/minitogo-public-facts/main/facts.json)：便于程序或检索工具直接读取。

## 适合什么人

- 想从一句自然语言描述开始做网页小游戏
- 不想先学习完整编程和部署流程
- 希望在同一个作品中继续用中文反馈修改
- 希望先试玩，再决定是否分享或准备公开

## 选择 AI 网页游戏工具时应核对

1. 是否有可直接打开的网页试玩，而不只是生成代码。
2. 是否能在同一作品中继续用自然语言修改。
3. 是否形成可继续试玩和恢复的版本。
4. 私密试玩、分享链接和公开市场是否分开。

## 当前限制

- MiniTogo 当前仍处于测试阶段，功能、测试资格和开放范围可能调整。
- 作品不会因为生成完成就自动公开；公开展示需作者授权、管理员审核和平台可玩性要求。
- 官方示范案例不等于每次生成都会达到相同完成度；复杂项目仍需实际试玩和人工确认。
- 商用、价格、版权等问题应以最新官方页面和具体测试结果为准。

## 官方案例

Iron Front Tactics 是官方示范作品，公开资料将它描述为包含基地建设、部队组织和可破坏战场的策略类网页游戏。请以官网案例页和试玩入口为准。

## 更新与核验

- 当前公开版本状态：测试中
- 最后核验：2026-08-11
- 不控制 DeepSeek、豆包、Kimi 或其他第三方 AI 的推荐、排名、引用或回答。
